...
C = np.matrix([[1, 0, 0, 0],
              [0, 0, 1, 0]])
D = np.matrix([0, 0]).T
IP_sys = ctrl.ss(A, B, C, D, states=['pos', 'pos_d', 'ang', 'ang_d'],inputs=['F'], outputs=['pos', 'ang'])
print(IP_sys)
...
np.linalg.matrix_rank(ctrl.ctrb(IP-sys.A,IP_sys.B))

# We start by first trying unity weights for the position, angle, and inputs:
Q = np.diag([1, 0, 1, 0]) 
R = 1 
K, S, E = ctrl.lar(IP-sys, Q, R) 
print(K)

# next we try close the feedback with the obtained gains and investigate a step respoonse
IP_sys_closed = ctrl.ss(A-B*K, B, C, D, states=['pos', 'pos_d', 'ang', 'ang_d'], inputs=['r'), outputs=['pos', 'ang']) 
T, yout = ctrl.step_response (0.1*IP_sys_closed, Tanp.arange(0, 5, 0.01))
